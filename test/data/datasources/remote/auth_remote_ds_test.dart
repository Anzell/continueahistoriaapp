...
void main() {
  ...
  group("signUp", () {
    ...
    test("should throw a ServerValidationException if response is validation_failure", () async {
      when(mockClient.post(any, body: anyNamed("body"), headers: {"Content-Type": "application/json"})).thenAnswer(
      ...
      when(mockClient.post(any, body: anyNamed("body"), headers: anyNamed("headers")))
          .thenAnswer((_) async => http.Response(
              json.encode({
                ...
                "result": {"id": "validId", "token": "validToken"}
              }),
              200));
      when(mockClient.get(any, headers: {"Content-Type": "application/json", "Authorization": "Bearer validToken"}))
          .thenAnswer((_) async => http.Response(
              json.encode({
                ...
                "message": "sucesso na operação",
                "code": ServerCodes.success,
                "result": {"id": "validId", "email": "email@email.com", "username": "username"}
              }),
              200));
  ...
  group("auto login", () {
    test("should return a valid user if authorization is setted", () async {
      ...
      when(mockHiveBox.get(HiveStaticKeys.userModel))
          .thenReturn({"id": "validId", "username": "username", "email": "email@valid.com"});
